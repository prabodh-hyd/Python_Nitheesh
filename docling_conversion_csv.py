...
for i, table in enumerate(conversion.document.tables):
    ...
    table_convert = table_convert.iloc[1:].reset_index(drop=True)
    element_csv_file = output_path / f"{doc_filename}-{i + 1}.csv"
    table_convert.to_csv(element_csv_file, index = False, header = False)
